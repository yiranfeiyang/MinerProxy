...
install_download() {
    ...
    git clone https://github.com/yiranfeiyang/web.git
...
start_write_config() {
    ...
    echo "下载完成，开启守护"
    echo
    chmod a+x $installPath/yyProxy_web
    if [ -d "/etc/supervisor/conf/" ]; then
        rm /etc/supervisor/conf/MinerProxy.conf -f
        echo "[program:MinerProxy]" >>/etc/supervisor/conf/MinerProxy.conf
        echo "command=${installPath}/yyProxy_web" >>/etc/supervisor/conf/MinerProxy.conf
        ...
        echo "command=${installPath}/yyProxy_web" >>/etc/supervisor/conf.d/MinerProxy.conf
        ...
        echo "command=${installPath}/yyProxy_web" >>/etc/supervisord.d/MinerProxy.ini
    ...
    echo "请以访问本机IP:18888"
    echo
    echo "安装完成...守护模式无日志，需要日志的请以nohup ./yyProxy_web &方式运行"
    echo
